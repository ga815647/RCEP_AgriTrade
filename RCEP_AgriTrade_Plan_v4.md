...
> - v3 → v3.1：台灣數據改從 BACI（M49=490）取用；架構統一為單一來源
> - v3.1 → v3.2：HS 對照表直連連結修正；版本號說明清理
> - v3.2 → **v4.0**：依實際 zip 內容精進程式架構（詳見第 1 節 v4 變更說明）
>
> ⚠️ **核心取捨**：台灣數據來自 BACI（M49=490，Chinese Taipei），BACI 已對
...
依據實際 zip 解壓後的檔案結構，進行以下改善：

| 項目 | v3.x 做法 | v4.0 改善 | 效果 |
|------|----------|----------|------|
| **country_codes** | 硬編碼於 `country_codes.py` | 從 BACI 附帶的 `country_codes_V*.csv` 讀取（三版本內容相同，只讀 hs17 那份）| 國家名稱自動與 BACI 同步 |
| **product_codes** | 未利用 | 從 `product_codes_HS{ver}_V*.csv` 讀取 HS6 英文說明，版本專屬 | 自動補入 HS6_Description_EN |
| **檔案路徑比對** | 硬編碼版本號（`V202601`）| 改用 `glob` 萬用字元（`BACI_{ver}_Y{year}_V*.csv`）| 版本號更新後免改程式 |
| **product_codes 路徑** | 固定檔名 | `glob("product_codes_HS{ver}_V*.csv")` | 同上 |
| **country_codes 路徑** | — | `glob("country_codes_V*.csv")`，取 hs17 資料夾第一個結果 | 同上 |
...
| HS 說明文字 | BACI 附帶 `product_codes_HS17_V202601.csv` | 隨 BACI HS17 zip 一起解壓 | **0** |
...
| BACI 版本 | 下載檔案 | 解壓後內容 | 本計畫使用年份 |
|-----------|---------|-----------|------------|
| **HS07** | `BACI_HS07_V202601.zip`（1 個）| Y2007–Y2024 的 CSV + country_codes + product_codes + Readme | 2010–2011 |
| **HS12** | `BACI_HS12_V202601.zip`（1 個）| Y2012–Y2024 的 CSV + country_codes + product_codes + Readme | 2012–2016 |
| **HS17** | `BACI_HS17_V202601.zip`（1 個）| Y2017–Y2024 的 CSV + country_codes + product_codes + Readme | 2017–2024 |

> 每個版本只有**一個 zip**，解壓後含所有年份。本計畫只會讀取需要的年份，其餘 CSV 可以保留或刪除。

**解壓後每個版本資料夾內的附帶檔案**（自動被程式使用）：
- `country_codes_V202601.csv`：M49 代碼與國家名稱對照（取代手動維護的 country_codes.py）
- `product_codes_HS17_V202601.csv`：HS6 代碼與英文說明（取代外部查詢）
...
**下載（直接連結，點擊即下載）**：
- `https://unstats.un.org/unsd/classifications/Econ/tables/HS2017toHS2007ConversionAndCorrelationTables.xlsx`
- `https://unstats.un.org/unsd/classifications/Econ/tables/HS2017toHS2012ConversionAndCorrelationTables.xlsx`

放入 `data/reference/` 資料夾，檔名保持原樣。

**反轉邏輯說明**：

```
原始檔案內容（HS2017 → HS2007）：
  HS2017代碼  | HS2007代碼
  030389      | 030389
  030441      | 030410（一對一）
  030443      | 030410（多對一：030443 和 030444 都來自 030410）
  030444      | 030410

反轉後（HS2007 → HS2017，本計畫需要的方向）：
  HS2007代碼  | HS2017代碼清單
  030389      | [030389]
  030410      | [030441, 030443, 030444]  ← 一個舊代碼對應多個新代碼，金額等比分配
```
...
| 步驟 | 動作 | 預估時間 |
|------|------|---------|
| **M1** | 下載 `BACI_HS07_V202601.zip`（1 個）| 10 分鐘 |
| **M2** | 下載 `BACI_HS12_V202601.zip`（1 個）| 15 分鐘 |
| **M3** | 下載 `BACI_HS17_V202601.zip`（1 個）| 20 分鐘 |
| **M4** | 各自解壓縮至 `data/raw/baci/hs07/`、`hs12/`、`hs17/` | 15 分鐘 |
| **M5** | 下載 HS 對照表 2 個 xlsx，放入 `data/reference/` | 5 分鐘 |

**總手動時間：約 1 小時（主要是下載等待）**
**不需要任何帳號或 API Key**
...
```
T+0:00   ✋ M1–M3：直接前往 CEPII 下載 BACI 三版本（3 個 zip，30–60 分鐘，可背景執行）
...
baci:
  ...
  # ↑ 使用萬用字元 * 匹配版本號，CEPII 更新版本號時無需修改程式碼
  # 程式用 glob 找到實際檔案，取第一個（通常只有一個）
  version_router:
    "2010-2011": "HS07"
    "2012-2016": "HS12"
    "2017-2024": "HS17"
...
> BACI zip 解壓後附有 `country_codes_V202601.csv`，程式應優先讀取此檔案
> 作為 M49↔國家名稱的對照來源，不要完全硬編碼。
> 但以下 `ALL_COUNTRIES` dict 仍需在程式中定義，作為過濾集合用途。
...
def build_baci_path(year: int, cfg: dict) -> str:
    """用 glob 匹配實際檔案，不依賴硬編碼版本號"""
    ver      = get_baci_version(year)
    baci_dir = cfg["baci"]["baci_dir"]
    pattern  = cfg["baci"]["filename_pattern"]  # "BACI_{version}_Y{year}_V*.csv"
    glob_pat = os.path.join(baci_dir, ver.lower(),
                            pattern.format(version=ver, year=year))
    matches  = glob.glob(glob_pat)
    if not matches:
        raise FileNotFoundError(
            f"找不到 BACI {ver} {year} 年檔案（搜尋：{glob_pat}）\n"
            f"請依 README 步驟 M1–M4 下載並解壓縮 BACI {ver} zip。"
        )
    return matches[0]
...
def load_baci_year(year: int, cfg: dict) -> pd.DataFrame:
    ...
    df   = df[df["i"].isin(ALL_M49) | df["j"].isin(ALL_M49)]
    df   = df[df["k"].str[:2].isin(AGR_CHAPTERS)]
    df["baci_version"] = get_baci_version(year)
    logger.info(f"[BACI] {year} ({get_baci_version(year)}) 讀入，農產品相關行：{len(df):,}")
    return df

def load_country_codes(cfg: dict) -> pd.DataFrame:
    """
    從 hs17 資料夾讀取 country_codes_V*.csv。
    三個版本的內容相同，只讀一份（hs17 為最新，優先使用）。
    """
    baci_dir = cfg["baci"]["baci_dir"]
    matches  = glob.glob(os.path.join(baci_dir, "hs17", "country_codes_V*.csv"))
    if not matches:
        logger.warning("找不到 country_codes_V*.csv，國家名稱將使用內建對照表")
        return pd.DataFrame()
    df = pd.read_csv(matches[0], dtype=str)
    logger.info(f"[Meta] country_codes 載入：{matches[0]}")
    return df

def load_product_codes(baci_version: str, cfg: dict) -> pd.DataFrame:
    """
    從對應版本資料夾讀取 product_codes_HS{ver}_V*.csv。
    每個版本有自己的 product_codes（HS07/HS12/HS17 代碼說明不同）。
    """
    baci_dir = cfg["baci"]["baci_dir"]
    matches  = glob.glob(os.path.join(baci_dir, baci_version.lower(),
                                      f"product_codes_{baci_version}_V*.csv"))
    if not matches:
        logger.warning(f"找不到 product_codes_{baci_version}_V*.csv，HS 說明將留空")
        return pd.DataFrame()
    df = pd.read_csv(matches[0], dtype=str)
    logger.info(f"[Meta] product_codes {baci_version} 載入：{matches[0]}")
    return df
```
...
def load_concordance(ref_dir: str) -> dict:
    """
    載入 UN Stats 對照表並反轉方向。
    
    原始檔案方向：HS2017 → 舊版本（HS2007 或 HS2012）
    本函式反轉為：舊版本 → HS2017（程式查詢時需要的方向）
    
    反轉邏輯：
      原始：HS2017代碼 A → [HS2007代碼 X, Y]
      反轉：HS2007代碼 X → [HS2017代碼 A]
            HS2007代碼 Y → [HS2017代碼 A]
    
    一個舊代碼可能對應多個 HS2017 代碼（原始檔案中多行指向同一舊代碼），
    金額後續需等比分配。
    """
    concordance = {}
    files = {
        "HS2007": "HS2017toHS2007ConversionAndCorrelationTables.xlsx",
        "HS2012": "HS2017toHS2012ConversionAndCorrelationTables.xlsx",
    }
    for src_ver, filename in files.items():
        path = os.path.join(ref_dir, filename)
        if not os.path.exists(path):
            logger.warning(f"找不到對照表 {filename}，{src_ver} 數據將保留原碼")
            continue

        df = pd.read_excel(path, dtype=str)

        # 找出「HS2017欄」和「舊版本欄」的欄位名稱
        # 原始 Excel 欄位名稱可能有空白或換行，用模糊匹配
        hs17_col = next(c for c in df.columns if "2017" in str(c))
        old_col  = next(c for c in df.columns if src_ver[-4:] in str(c))  # "2007" 或 "2012"

        df = df[[hs17_col, old_col]].dropna()
        df.columns = ["hs2017", "old"]
        df = df.astype(str).applymap(str.strip)

        # 反轉：以舊代碼為 key，HS2017 代碼清單為 value
        inverted = {}
        for _, row in df.iterrows():
            old_code  = row["old"].zfill(6)
            new_code  = row["hs2017"].zfill(6)
            inverted.setdefault(old_code, [])
            if new_code not in inverted[old_code]:
                inverted[old_code].append(new_code)

        concordance[src_ver] = inverted
        logger.info(f"[HS] {src_ver} 對照表載入，{len(inverted)} 個舊代碼")
...
def harmonize_row(hs6_orig: str, baci_version: str, concordance: dict) -> list[dict]:
    ...
    mapping = concordance.get(src_ver, {}).get(hs6_orig.zfill(6))
    if not mapping:
        return [{"HS6_Code": hs6_orig, "weight": 1.0,
...
### M1–M3（30–60 分鐘）：下載 BACI 三版本

前往 `https://www.cepii.fr/CEPII/en/bdd_modele/bdd_modele_item.asp?id=37`
**不需要註冊帳號**，直接下載最新版本的三個 zip：

| 選擇版本 | 下載檔案 | 大小（約）|
|---------|---------|---------|
| **BACI HS07** | `BACI_HS07_V202601.zip`（1 個）| ~1 GB |
| **BACI HS12** | `BACI_HS12_V202601.zip`（1 個）| ~1.5 GB |
| **BACI HS17** | `BACI_HS17_V202601.zip`（1 個）| ~2 GB |

### M4（15 分鐘）：解壓縮，放入對應資料夾

每個 zip 解壓後含有所有年份 CSV 和附帶參考檔案：
```
data/raw/baci/hs07/  ← 解壓 BACI_HS07_V202601.zip 的全部內容
data/raw/baci/hs12/  ← 解壓 BACI_HS12_V202601.zip 的全部內容
data/raw/baci/hs17/  ← 解壓 BACI_HS17_V202601.zip 的全部內容
```

> 程式只讀取計畫年份範圍內的 CSV，多餘年份的 CSV 不影響執行。
> `country_codes_V202601.csv` 和 `product_codes_HS*.csv` 會由程式自動讀取。

### M5（5 分鐘）：下載 HS 版本對照表

### M5（5 分鐘）：下載 HS 版本對照表

直接點擊以下兩個連結下載，放入 `data/reference/`：
...
> ⚠️ 這兩個檔案的方向是 HS2017 → 舊版本，程式載入時會自動反轉，無需手動處理。
...
| 風險 | 發生機率 | 影響 | 備用方案 |
|------|---------|------|---------|
| **CEPII 版本號更新**（V202601→V202701）| 高（每年）| — | v4 使用 glob 萬用字元，**版本號更新後無需任何修改**，直接解壓新 zip 即可 |

...
2. **參數設定**（左側邊欄）：
   - **📅 時間範圍**：雙端滑桿，範圍與預設值由 `config.yaml` 的 `time_range.start` / `time_range.end` 動態決定（預設 2007–2024）
...
**系統處置**：`config.yaml` 提供 `exclude_hs6` 排除清單機制。研究者可自行決定是否排除轉口性質強的品項：

```yaml
exclude_hs6:
  codes: ["220820", "240220"]   # 排除白蘭地與香菸
  reason: "轉口貿易嫌疑"
```
...
2. **品項個別排除**：在 `config.yaml` 的 `exclude_hs6.codes` 中指定
...
```yaml
time_range:
  start: 2007              # 分析起始年份（GUI 滑桿預設左端）
  end: 2024                # 分析終止年份（GUI 滑桿預設右端）
...
exclude_hs6:
  codes: []                        # 排除品項，如 ["220820", "240220"]（詳見 §6.2 風險 1）
